...
import {
  ...
  setFiltroStatus,
  limparFiltros
} from "@/app/redux/slices/filtroOcorrenciaSlice";
...
export default function ChamadosPage() {
  ...
  const filtroGravidade = useSelector(
    (state: RootState) => state.filtroOcorrencia.gravidade
  );
  ...
  const ocorrenciasFiltradas = ocorrencias.filter((ocorrencia) => {
    const passaGravidade =
      filtroGravidade === "TODAS" || ocorrencia.gravidade === filtroGravidade;

    const passaStatus =
      filtroStatus === "TODOS" || ocorrencia.status === filtroStatus;

    return passaGravidade && passaStatus;
  });
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F6] p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-4 mb-6 flex flex-col md:flex-row gap-4">
          <div>
            <label className="block text-xs font-black uppercase text-stone-400 mb-2">
              Filtrar por gravidade
            </label>
            <select
              value={filtroGravidade}
              onChange={(e) => dispatch(setFiltroGravidade(e.target.value))}
              className="bg-stone-100 text-stone-700 text-sm font-bold py-3 px-4 rounded-xl border-none outline-none"
            >
          ...
          <div>
            ...
            <select
              value={filtroStatus}
              onChange={(e) => dispatch(setFiltroStatus(e.target.value))}
              className="bg-stone-100 text-stone-700 text-sm font-bold py-3 px-4 rounded-xl border-none outline-none"
            >
          ...
          <div className="flex items-end">
            <button
              ...
              className="bg-stone-200 text-stone-700 text-sm font-bold py-3 px-4 rounded-xl hover:bg-stone-300 transition"
            >
              Limpar filtros
            </button>
          </div>
              ...
              <tbody className="divide-y divide-stone-100">
                ...
                {ocorrenciasFiltradas.length === 0 && (
                  <tr>
                    <td
                      colSpan={5}
                      className="px-8 py-10 text-center text-stone-400 font-semibold"
                    >
